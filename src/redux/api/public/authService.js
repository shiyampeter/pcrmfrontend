...
export const userLogin = createAsyncThunk('userLogin', async (formData, thunkAPI) => {
    try {
...
export const publicEditAddress = createAsyncThunk('publicEditAddress', async ({ id, params }, thunkApi) => {
    try {
        const response = await PUBLICSERVER.put(`/customer/address/${id}`, params);
        return response.data;
    }
    catch (error) {
        return thunkApi.rejectWithValue(error);
